...
type VersionRecord = { id: string; name: string; version: string; status: string; content_hash?: string; purpose?: string; dataset_id?: string };
...
function App() {
  ...
  return <div className="shell">
    ...
    <main><header><div><p className="eyebrow">PROJECT / {selected?.task_kind || "SETUP"}</p><h1>{selected?.name || "Vision AI Lifecycle"}</h1>{selected && <small className="muted">{selected.mode === "guided" ? "Guided practice · 자료와 결과를 단계별로 연결" : "User project · 직접 연결"}</small>}</div><div className="header-actions"><button className="secondary" disabled={isStatic} onClick={() => setProjectId(undefined)}>새 프로젝트</button><select aria-label="프로젝트 선택" value={projectId || ""} onChange={(event) => void loadProject(event.target.value)}><option value="">프로젝트 선택</option>{projects.map((project) => <option value={project.id} key={project.id}>{project.name}</option>)}</select></div></header>
      ...
      {page === "모델" && <><section><h2>모델 Registry</h2><p>가중치, config, 전후처리, class mapping을 하나의 model bundle로 관리합니다. 학습 데이터는 자동 연결하지 않습니다.</p><ModelTable models={models} onArchive={(id) => void archive("models", id)}/></section><ModelConnect projectId={projectId} datasets={datasets} onSaved={() => void loadProject(projectId)} onError={setMessage}/><ModelPreview projectId={projectId} models={models} onError={setMessage}/></>}
      ...
      {page === "데이터" && <><section><h2>Dataset Version</h2><p>먼저 서버/NAS 경로를 검사한 다음 검사 결과를 초안 DatasetVersion으로 저장합니다. 확정 버전은 수정하지 않고 새 버전을 만듭니다.</p><DatasetTable datasets={datasets} onArchive={(id) => void archive("datasets", id)} onFinalize={(id) => void finalizeDataset(id)}/><DatasetConnect projectId={projectId} onSaved={() => void loadProject(projectId)} onError={setMessage}/></section><DatasetPreview projectId={projectId} datasets={datasets} onError={setMessage}/><VersionManager projectId={projectId} datasets={datasets} versions={versions} onSaved={() => void loadProject(projectId)} onError={setMessage}/><PathInspector projectId={projectId} onError={setMessage}/></>}
      {page === "가이드" && <Guide />}
      {page === "실행·보드" && <><section><h2>실행·보드</h2><p>보드와 runtime은 버전이 있는 Target Profile로 기록합니다. 실제 benchmark manifest에는 해당 profile ID를 연결합니다.</p><TargetTable targets={targets}/>{!isStatic && <TargetConnect projectId={projectId} onSaved={() => void loadProject(projectId)} onError={setMessage}/>}</section><section><h2>등록 작업</h2><p>등록 runner만 실행할 수 있습니다. 모의 board runner는 result contract 검증용입니다.</p><button disabled={isStatic} onClick={() => void runMockBoard()}>모의 보드 실행</button><JobTable jobs={jobs} onCancel={(id) => void cancelJob(id)} onRetry={(id) => void retryJob(id)}/></section></>}
...
function JobTable({ jobs, onCancel, onRetry }: { jobs: Job[]; onCancel?: (id: string) => void; onRetry?: (id: string) => void }) { return <div className="tablewrap jobs"><table><thead><tr><th>Runner</th><th>Status</th><th>Log</th><th>작업</th></tr></thead><tbody>{jobs.length ? jobs.map((job) => <tr key={job.id}><td>{job.runner_id}</td><td><span className="pill">{job.status}</span></td><td>{job.log || "대기 중"}</td><td>{onCancel && ["queued", "running"].includes(job.status) && <button className="table-action" disabled={isStatic} onClick={() => onCancel(job.id)}>취소</button>}{onRetry && ["failed", "timed_out", "cancelled", "interrupted"].includes(job.status) && <button className="table-action" disabled={isStatic} onClick={() => onRetry(job.id)}>재시도</button>}</td></tr>) : <tr><td colSpan={4}>등록된 작업이 없습니다.</td></tr>}</tbody></table></div>; }
