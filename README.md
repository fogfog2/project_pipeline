...
- Dataset 목록에 저장된 source validation 상태를 표시해 draft·검증 완료·미검증 상태를 구분
- ONNX/MMDeploy 모델은 명시적 inference profile이 없으면 `runnable=false`와 `adapter_status=required`로 등록
- 모델 경로·metadata 수정 시에도 같은 profile 검증을 적용해 실행 가능 상태를 우회할 수 없음
- ONNX profile의 task, input size, layout, color, input name 구조를 등록·수정 단계에서 검증
...
- 독립 worker queue는 조건부 claim으로 중복 실행을 막고 worker lease owner·만료 시각·시도 횟수를 작업 화면과 API에 기록하며, 실행 중 heartbeat 갱신과 만료 실행 재큐잉을 제공
...
- Release gate의 `critical_classes`로 `details.per_class`의 클래스별 지표(minimum/maximum)를 검증하며 해당 클래스 지표가 없으면 `INCOMPLETE`
...
최신 제품 설계와 사용자 흐름은 [빈 프로젝트에서 시작하는 실습형 Lifecycle 계획](docs/guided-lifecycle-plan.md)이다. 현재 구현에는 빈 프로젝트·가이드 단계, 실제 연결·평가·변경 비교, 수정/보관, 디렉터리 연결, cursor 기반 작업 로그와 readiness 표시가 포함되어 있다.
