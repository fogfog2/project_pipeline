...
추가 반영: EvaluationSet의 `items`/`image_ids`를 실제 분류·COCO·ONNX 평가에 적용하고 중복·누락·빈 세트 validation과 재검사 UI를 제공한다. 온보딩은 단계별 readiness와 막힌 이유를 반환하며, Pages workflow는 demo 또는 커밋된 선택 프로젝트 export snapshot을 검증해 게시한다.
