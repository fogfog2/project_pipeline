...
| S17 | §14: 다단계 release gate·승인 | 부분 구현·정확성 보완 필요 | 단일 평가 scalar 규칙과 baseline dataset/evaluator/protocol/scope/class mapping 호환성 검사를 제공하며, Release 생성 시 evaluation/board/quantization/artifact evidence snapshot/hash, 종류별 required evidence와 `required_evidence_refs`의 특정 ID 누락 INCOMPLETE를 기록한다. 주요 registry 변경의 local AuditEvent 이력은 제공하지만 다중 세트·critical class·인증된 승인자·승인 워크플로는 남아 있다 |
