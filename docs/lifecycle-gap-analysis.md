...
| S16 | §13: 외부 작업 실행·취소·복구 | 부분 구현 | subprocess runner와 독립 worker 모드, 재시작 복구, 전체 로그·exit code 표시, 외부 worker 재시도 queue 보존을 제공한다. 취소 의도를 먼저 저장하고 POSIX process group을 종료하며 timeout도 하위 프로세스까지 정리한다. API와 다른 worker가 프로세스를 소유해도 DB `cancelling` marker를 worker watcher가 감지한다. 실시간 websocket 로그·취소 lease/다중 worker 경쟁 제어 보완이 남아 있다. `runner.py`, `main.py` |
