...
| S14 | §12,17: baseline/candidate 공식 비교 | 부분 구현·정확성 보완 필요 | 완료된 평가 중 동일 dataset·EvaluationSet·전체 평가 config·evaluator·protocol·class mapping 계약을 만족하는 최신 pair를 선택하고 latency 계약을 별도 검사한다. 비교 응답과 UI에 계약별 matched/mismatch 상태를 표시한다. 다중 세트·slice·전체 설정 hash/승인 이력은 남아 있다. `service.py:compare_models` |
