...
| S05 | §7: Core/Field/Hard/Regression 고정 평가 세트 | 부분 구현 | EvaluationSetVersion을 평가 API/Run.config에 명시적으로 연결하고 Dataset 불일치를 거부하며 비교 계약에 포함한다. definition의 `items`/`image_ids`를 분류·COCO·ONNX batch 입력에 실제 적용하고 선택 개수/평가 레코드 수를 결과에 남긴다. 다중 set 보고서와 slice별 리포트는 남아 있다 |
