...
구현 현황(2026-09-19): U0/U1과 U2의 시작점을 반영했다. 빈 프로젝트/선택형 실습, 초안 등록·보관·참조 없는 초안 삭제, storage mapping의 읽기 전용 탐색·재검사·root 재연결 후 asset hash 상태 갱신, DataAsset 파일 inventory와 대용량 inventory Job, annotation/manifest fingerprint와 변경 감지, label/split/evaluation/calibration contract, COCO bbox preview, typed QuantizationRun·BoardBenchmark, lineage graph, recipe별 onboarding session/step evidence, detection COCO 및 classification prediction records 평가, 외부 COCO prediction 평가 및 Release gate UI, 주요 registry 변경의 AuditEvent API/UI까지는 동작한다. 실제 Storage/Dataset/Model/Run/Release 근거가 없으면 onboarding 완료를 거부한다. 서비스 재시작 시 active job을 interrupted로 보존하고 실패·timeout·취소·interrupted 작업을 명시적으로 재시도하는 처리도 동작한다.
...
| 비교 결과는 상단 메시지, Release는 API 주소 표시 | 클래스별 개선·회귀를 탐색하거나 결정 기록 불가 | 비교 report 표·계약 사유·JSON 다운로드와 증거 기반 Release 폼 |
| 메뉴 선택은 React 메모리 상태 | 새로고침·뒤로 가기·상세 링크 공유 불가 | `#/projects/:id/:page` hash routing으로 프로젝트·화면을 복원하고 Pages 하위 경로에서도 조회 |
...
EvaluationSet의 `definition`에 `items` 또는 `image_ids` 배열을 넣으면 평가기는 그 목록에 포함된 레코드/COCO image만 사용한다. 목록을 생략하면 연결된 Dataset 전체를 사용하고, 빈 배열은 빈 평가 세트로 취급해 성공으로 포장하지 않는다. 결과에는 선택 개수와 실제 평가 레코드 수를 기록하므로 사용자가 입력 manifest 전체를 평가했다고 오해하지 않도록 한다.
